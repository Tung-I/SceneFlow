...
from lib.pwc_modules.modules import (WarpingLayer, WarpingLayer3D, FeaturePyramidExtractor, CostVolumeLayer, CostVolumeLayer3D, SceneFlowEstimator, SceneFlowContextNetwork, Generate3DFeature)
# from correlation_package.modules.correlation import Correlation


class PWCRGBD3Net(nn.Module):


    def __init__(self, device, lv_chs, search_range, batch_norm, corr_activation, residual, output_level, depth_range):
        super(PWCRGBD3Net, self).__init__()
        self.device = device
        self.lv_chs = lv_chs
        ...
        for l, ch in enumerate(lv_chs[::-1]):
            layer = SceneFlowEstimator(ch + (search_range*2+1)**3 + 3, batch_norm).to(device)
    ...
    def forward(self, x1_raw, x2_raw, x1_disp_raw, x2_disp_raw):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            x1_3d = self.generate_3d_feature(x1, x1_disp)
            x2_3d = self.generate_3d_feature(x2, x2_disp)

            b, c, d, h, w = x1_3d.size()
            flow_3d = torch.zeros(b, 3, d, h, w).to(self.device)
            for i in range(d):
                flow_3d[:, :, i, :, :] = flow[:, :, :, :]

            x2_3d_warp = self.warping_layer(x2_3d, flow_3d)
    
            # correlation
            corr_3d = self.corr(x1_3d, x2_3d_warp)
            corr = torch.mean(corr_3d, 2).squeeze_()
