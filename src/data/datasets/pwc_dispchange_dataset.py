...
class PWCDispchangeDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        dispchange = flow[:, :, 2]
        dispchange = np.expand_dims(dispchange, axis=2) # (H, W, C)
        ...
        rgb_l, rgb_r = self.transforms(rgb_l, rgb_r, dtypes=[torch.float, torch.float])
        rgb_next_l, rgb_next_r = self.transforms(rgb_next_l, rgb_next_r, dtypes=[torch.float, torch.float])
        dispchange = self.transforms(dispchange, dtypes=[torch.float])
        ...
        dispchange = dispchange.permute(2, 0, 1).contiguous()
        ...
        return {"rgb_l": rgb_l, "rgb_r": rgb_r, "rgb_next_l": rgb_next_l, "rgb_next_r": rgb_next_r, "dispchange": dispchange}
