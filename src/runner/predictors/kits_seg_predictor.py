...
class KitsSegPredictor(object):
    """The predictor for the Kits2019 segmentation task.
    Args:
    ...
    def predict(self):
        ...
        with open(self.data_split_csv, "r") as f:
            ...
            for case_name, split_type in rows:
                if split_type == 'Validation':
                    image_path = self.data_dir / case_name / 'imaging.nii.gz'
                    label_path = self.data_dir / case_name / 'segmentation.nii.gz'
        ...
        for data_path in trange:
            ...
            if self.exported:
                filename = str(image_path.parts[-2])
