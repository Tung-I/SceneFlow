...
class PWCRGBDTrainer(BaseTrainer):
    ...
    def _run_epoch(self, mode):
        ...
        for batch in trange:
            ...
            rgb_l, rgb_next_l, disp, disp_next, flow = self._get_inputs_targets(batch)
            target = {'flow':flow}


            if mode == 'training':
                outputs = self.net(rgb_l, rgb_next_l, disp, disp_next)
                # print(outputs.size())
    ...
    def _get_inputs_targets(self, batch):
        ...
        """
        return batch['rgb_l'], batch['rgb_next_l'], batch['disparity'], batch['disparity_next'], batch['flow']
